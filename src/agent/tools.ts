...
/** Sanitize float values - replace Infinity, -Infinity, NaN with 0 */
...
/** Sanitize an entire object recursively - replace all Infinity/NaN with 0 */
function sanitizeObject(obj: any): any {
    if (obj === null || obj === undefined) return obj;
    if (typeof obj === 'number') return safeFloat(obj);
...
export const TOOL_DEFINITIONS = [
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    address: {
                        type: 'string',
                        description: 'Wallet address. Bitcoin: starts with 1, 3, or bc1. Ethereum: starts with 0x, 42 chars.',
                    },
                    limit: {
                        type: 'number',
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Search cached wallet transactions with filters. Use after fetch_wallet_data.',
            parameters: {
                type: 'object',
                properties: {
                    address: { type: 'string', description: 'Wallet address to search transactions for' },
                    direction: { type: 'string', enum: ['incoming', 'outgoing'] },
                    min_value_btc: { type: 'number', description: 'Minimum transaction value in BTC (Bitcoin only)' },
                    max_value_btc: { type: 'number', description: 'Maximum transaction value in BTC (Bitcoin only)' },
                    min_value_eth: { type: 'number', description: 'Minimum transaction value in ETH (Ethereum only)' },
                    max_value_eth: { type: 'number', description: 'Maximum transaction value in ETH (Ethereum only)' },
                    time_range: {
                        type: 'object',
                        properties: {
                    ...
                    sort_by: { type: 'string', enum: ['time', 'value'] },
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Get cached summary for a wallet address including balance, totals, transaction count, and token_summary.',
            parameters: {
                type: 'object',
                properties: {
                    address: { type: 'string', description: 'Wallet address (Bitcoin or Ethereum)' },
                },
                required: ['address'],
            },
        },
    },
    {
        ...
        function: {
            ...
            description: 'Analyze cached transactions for anomalous patterns: large transactions, rapid sequences, round-number transactions, dormant wallet reactivation, fan-out/fan-in patterns.',
            parameters: {
                type: 'object',
                properties: {
                    address: { type: 'string', description: 'Wallet address to analyze (Bitcoin or Ethereum)' },
...
async function fetchBitcoinWallet(address: string, limit: number, now: string, startTime: number) {
    ...
    const transactions: WalletTransaction[] = apiData.txs.map(tx => {
        const inputAddrs = tx.inputs.map(i => i.prev_out?.addr).filter((a): a is string => !!a);
        const outputAddrs = tx.out.map(o => o.addr).filter((a): a is string => !!a);
        ...
        return {
            ...
            fee_satoshis: tx.fee,
            fee_btc: safeFloat(satoshiToBTC(tx.fee)),
            result_satoshis: tx.result,
            result_btc: safeFloat(satoshiToBTC(tx.result)),
            direction: isIncoming ? 'incoming' as const : 'outgoing' as const,
            value_satoshis: valueAbs,
            value_btc: safeFloat(satoshiToBTC(valueAbs)),
    ...
    const firstTx = transactions.length > 0 ? transactions.reduce((a, b) => a.time < b.time ? a : b) : null;
    const lastTx = transactions.length > 0 ? transactions.reduce((a, b) => a.time > b.time ? a : b) : null;
    ...
    const summary: WalletSummary = {
        ...
        total_received_btc: safeFloat(satoshiToBTC(apiData.total_received)),
        total_sent_satoshis: apiData.total_sent,
        total_sent_btc: safeFloat(satoshiToBTC(apiData.total_sent)),
        final_balance_satoshis: apiData.final_balance,
        final_balance_btc: safeFloat(satoshiToBTC(apiData.final_balance)),
...
async function fetchEthereumWallet(address: string, limit: number, now: string, startTime: number) {
    ...
    const normalTransactions: WalletTransaction[] = ethTxs.map(tx => {
        ...
        const valueEth = safeFloat(weiToETH(tx.value));
        const gasUsed = parseInt(tx.gasUsed, 10);
        const gasPrice = parseInt(tx.gasPrice, 10);
        const feeWei = gasUsed * gasPrice;
        const feeEth = safeFloat(weiToETH(feeWei.toString()));
        ...
        return {
            ...
            value_eth: valueEth,
            gas_used: gasUsed,
            gas_price_gwei: safeFloat(gasPrice / 1e9),
            fee_eth: feeEth,
    ...
    const tokenTransactions: WalletTransaction[] = tokenTxs
        .filter(tx => !normalTxHashes.has(tx.hash))
        .map(tx => {
            const isIncoming = tx.to.toLowerCase() === addressLower;
            const tokenVal = safeFloat(tokenToDecimal(tx.value, tx.tokenDecimal));
            const timestamp = parseInt(tx.timeStamp, 10);
    ...
    for (const tx of normalTransactions) {
        ...
        if (tokenTx && (tx.value_eth === 0 || tx.value_eth === undefined)) {
            ...
            tx.token_value = safeFloat(tokenToDecimal(tokenTx.value, tokenTx.tokenDecimal));
    ...
    const totalReceivedEth = safeFloat(incoming.reduce((sum, tx) => sum + safeFloat(tx.value_eth), 0));
    const totalSentEth = safeFloat(outgoing.reduce((sum, tx) => sum + safeFloat(tx.value_eth), 0));

    // Build per-token breakdown
    const tokenMap = new Map<string, {
        symbol: string; name: string; contract: string;
        total_in: number; total_out: number; tx_count: number;
    }>();
        ...
        if (tx.is_token_transfer && tx.token_symbol) {
            ...
            if (!tokenMap.has(key)) {
                tokenMap.set(key, {
                    ...
                    name: tx.token_name || tx.token_symbol,
                    contract: tx.token_contract || '',
                    total_in: 0,
                    total_out: 0,
                    tx_count: 0,
                });
            ...
            entry.tx_count++;
            const tokenVal = safeFloat(tx.token_value);
            if (tx.direction === 'incoming') {
                entry.total_in = safeFloat(entry.total_in + tokenVal);
            } else {
                entry.total_out = safeFloat(entry.total_out + tokenVal);
            }
        }
    }
    ...
    const ethPriceUsd = safeFloat(prices.get('eth') || 0);

    const tokenSummary = Array.from(tokenMap.values())
        .map(t => {
            const priceUsd = safeFloat(prices.get(t.contract.toLowerCase()) || 0);
            const total_in = safeFloat(t.total_in);
            const total_out = safeFloat(t.total_out);
            const total_in_usd = priceUsd ? safeFloat(Number((total_in * priceUsd).toFixed(2))) : undefined;
            const total_out_usd = priceUsd ? safeFloat(Number((total_out * priceUsd).toFixed(2))) : undefined;
            return {
                symbol: t.symbol,
                name: t.name,
                contract: t.contract,
    ...
    const balanceEth = safeFloat(weiToETH(balanceWei));
    ...
    const summary: WalletSummary = {
        ...
        final_balance_eth: balanceEth,
        total_received_eth: totalReceivedEth,
        total_sent_eth: totalSentEth,
        eth_price_usd: ethPriceUsd || undefined,
        total_received_usd: ethPriceUsd ? safeFloat(Number((totalReceivedEth * ethPriceUsd).toFixed(2))) : undefined,
        total_sent_usd: ethPriceUsd ? safeFloat(Number((totalSentEth * ethPriceUsd).toFixed(2))) : undefined,
        balance_usd: ethPriceUsd ? safeFloat(Number((balanceEth * ethPriceUsd).toFixed(2))) : undefined,
    ...
    // Final sanitize entire summary and transactions before indexing
    const sanitizedSummary = sanitizeObject(summary);
    const sanitizedTransactions = transactions.map(tx => sanitizeObject(tx));

    await indexTransactionsAndSummary(address, sanitizedTransactions, sanitizedSummary);

    return {
        chain: 'ethereum',
        summary: sanitizedSummary,
        transactions_fetched: transactions.length,
        sample_transactions: sanitizedTransactions.slice(0, 10).map((tx: any) => ({
            tx_hash: tx.tx_hash,
            direction: tx.direction,
...
async function executeSearchTransactions(params: z.infer<typeof SearchTransactionsParamsSchema>) {
    ...
    const mustClauses: any[] = [{ term: { address: params.address } }];

    if (params.direction) mustClauses.push({ term: { direction: params.direction } });
    ...
    if (params.time_range) {
        mustClauses.push({ range: { time_iso: { gte: params.time_range.from, lte: params.time_range.to } } });
    }
    ...
    return { chain, results, total, elasticsearch_query: esQuery.body, execution_time_ms: executionTime };
}
...
async function executeGetWalletSummary(params: z.infer<typeof GetWalletSummaryParamsSchema>) {
    ...
    try {
        const response = await esClient.get({ index: WALLET_SUMMARY_INDEX, id: params.address });
        return { summary: response._source, found: true, execution_time_ms: Date.now() - startTime };
    } catch (error: any) {
...
async function executeDetectAnomalies(params: z.infer<typeof DetectAnomaliesParamsSchema>) {
    ...
    const response = await esClient.search({
        index: WALLET_TX_INDEX,
        body: { query: { term: { address: params.address } }, sort: [{ time: 'asc' }], size: 10000 },
    });
    ...
    const getValueField = (tx: WalletTransaction): number => safeFloat(chain === 'bitcoin' ? (tx.value_btc || 0) : (tx.value_eth || 0));
    ...
    const mean = safeFloat(values.reduce((a, b) => a + b, 0) / values.length);
    const stddev = safeFloat(Math.sqrt(values.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / values.length));
    const largeThreshold = safeFloat(mean + thresholds.stddevMultiplier * stddev);
    ...
    const roundTxs = transactions.filter(tx => {
        ...
        if (chain === 'bitcoin') return val >= 0.1 && (val % 0.1 < 0.0001 || (0.1 - val % 0.1) < 0.0001);
        else return val >= 0.1 && (val % 0.1 < 0.001 || (0.1 - val % 0.1) < 0.001);
    });
    ...
    if (chain === 'ethereum') {
        ...
        if (failedTxs.length > 0) {
            anomalies.push({
                ...
                severity: 'medium',
                description: `${failedTxs.length} failed transaction(s) detected.`,
        ...
        if (gasValues.length > 0) {
            const avgGas = safeFloat(gasValues.reduce((a, b) => a + b, 0) / gasValues.length);
            const highGasTxs = transactions.filter(tx => safeFloat(tx.gas_price_gwei || 0) > avgGas * 3);
            if (highGasTxs.length > 0) {
                anomalies.push({
                    type: 'high_gas_price',
                    severity: 'low',
                    description: `${highGasTxs.length} transaction(s) with gas price 3x above average (avg: ${avgGas.toFixed(2)} Gwei).`,
