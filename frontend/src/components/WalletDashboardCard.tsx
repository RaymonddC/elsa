...
export default function WalletDashboardCard({ address, stats, isLoading }: WalletDashboardCardProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-[20px] bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] p-6 transition-all duration-300 hover:bg-white/[0.06] hover:border-white/[0.08] cursor-default">
      {/* Gradient overlay on hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#F59E0B]/5 via-transparent to-[#8B5CF6]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>

      <div className="relative z-10">
        {/* Header: Wallet Address */}
        <div className="mb-6">
          <p className="text-white/30 text-[11px] uppercase tracking-widest font-semibold mb-2">Wallet Address</p>
          <p className="text-white/70 text-[13px] font-mono break-all">{address}</p>
        </div>

        {/* Main Balance */}
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="text-white/30 text-[11px] uppercase tracking-widest font-semibold mb-1">Total Balance</p>
            <h2 className="text-[36px] font-bold text-white/90 tracking-tight leading-none">
        ...
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/[0.06]">
          ...
          <div className="group/stat">
            <div className="flex items-center gap-2 mb-2">
              <Activity className="w-3.5 h-3.5 text-white/40" strokeWidth={2} />
              <p className="text-white/40 text-[11px] uppercase tracking-wider font-medium">Transactions</p>
            </div>
            <p className="text-white/85 text-[18px] font-semibold">{formatNumber(transactionCount)}</p>
          </div>
          ...
          <div className="group/stat">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-white/40 text-[11px] uppercase tracking-wider font-medium">Risk Score</p>
            </div>
          ...
          {/* Last Activity */}
          <div className="group/stat">
            <p className="text-white/40 text-[11px] uppercase tracking-wider font-medium mb-2">Last Activity</p>
            <p className="text-white/85 text-[14px] font-medium">{lastActivity || 'N/A'}</p>
          </div>
        </div>
      </div>
